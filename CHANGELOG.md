...
- **One exception: a bind mount the daemon created.** `./data:/data` on a
  host where Docker made the directory root-owned never worked either, but it
  used to start, go green and die at the first registration. It now refuses to
  start and names the directory. Fix the ownership (the image runs as `node`,
  uid 1000) rather than the config.
- **A write that fails after boot** is visible in two places: one stderr line
  on the transition, and `/healthz` gains `"persistence": "degraded"` until a
  write succeeds again. Anything that restarts on health can now see it before
  the restart drops every registration and token.
...
Nothing here changes the tool surface. It is still 45 tools and `tools/list` is byte-identical, because prompts and resources are separate lists over separate protocol methods.

### Upgrading from 3.1

- **Drop-in.** No config changes, no new arguments, no tools added or removed.
- **New in your client:** three slash commands (`troubleshoot_application`, `explain_failed_deploy`, `estate_health`) and two attachable resources (`coolify://overview` and `coolify://application/{uuid}`, instance-scoped in fleet mode). Client support for prompts and resources varies; a client that implements neither loses nothing, because every workflow is still reachable through the tools.
- **`search_docs` no longer depends on coolify.io.** The index ships inside the package and refreshes in the background, so the first search on a cold start is instant and an outage is not a failure.
- **HTTP mode: nothing to do.** `/register` still works for clients that need it. Clients supporting Client ID Metadata Documents now use those instead, which stops the OAuth state file growing by one client per fresh connection.
- **Read-only mode is more honest.** `_actions` no longer suggests tools a read-only server does not register, and `explain_failed_deploy` is not offered there because its tool is absent.
...
- **Client ID Metadata Documents in HTTP mode** (#340). The authorization server advertises `client_id_metadata_document_supported` and accepts an https URL as `client_id`, fetching the client's registration from that URL through the SSRF guard (public addresses only, pinned DNS, no redirects, 8 KB and 10 s caps) and validating it per the draft: the document's `client_id` must equal the URL by simple string comparison, `redirect_uris` follow the same rules as dynamic registration, and no shared-secret authentication method or `client_secret` is allowed. Documents are cached in memory for an hour and never persisted; fetch and validation errors are never cached. The consent page shows the hostname the identifier resolves to. Dynamic client registration at `/register` is unchanged, for clients that predate this, but the spec deprecates it and Claude prefers the document, which also stops the state volume growing by one client per fresh connection.
- **`search_docs` works offline** (#372). The Coolify docs index (`llms.txt`, 271 pages, 46 KB) now ships inside the package as `src/data/coolify-docs.json` and is served immediately; one background refresh per process pulls the live file with a five-second timeout and swaps it in if it parses, so a search never waits on coolify.io and never fails because of it. `npm run docs:index` refreshes the bundle, the publish workflow refreshes it at release time, and a weekly workflow opens a PR when the committed copy falls behind the live site. The bundle is validated with the same parser the server runs (`src/lib/docs-index-parse.mjs`), so a file the script accepts is a file the server can read; a bundle that is missing or unreadable is healed from the live index once, in the foreground, and otherwise every search names the rebuild command. Behaviour when online is unchanged except faster; a live file that parses to nothing, or only partly, is logged once instead of silently serving an empty index.
- **Prompts and resources (#371).** The server now exposes all three MCP primitives, not just tools. Three prompts ship as guided workflows a client shows as slash commands: `troubleshoot_application` (status and log tail, then configuration, ending in a cause and the smallest fix), `explain_failed_deploy` (which stage broke, quoting the evidence) and `estate_health` (known issues plus a status sweep, worst first). Two resources a client can attach: `coolify://overview` and `coolify://application/{uuid}`, instance-scoped in fleet mode. Prompts make no API call — pre-fetching would stall a slash command and would put attacker-influenceable build output in a user-role message, outside the untrusted-data boundary the tool layer puts around it. Resource reads go through the same client as every tool call, so the sanitizer applies and no URI takes a `reveal`. A prompt whose tools are not registered is not listed at all, so read-only mode drops `explain_failed_deploy` rather than offering a dead end. Separate lists over separate methods: the tools/list token budget is unchanged. [Docs](docs/prompts-and-resources.md).
...
- **Coolify OpenAPI spec re-vendored** (#347) against upstream `main` as of 2026-09-09: 189 → 275 operations, `docs/openapi-chunks/` regenerated. No client route changed shape and no tool changed. The spec now documents `GET /team` in place of `GET /teams/current`; the old path is still routed upstream and is the only one that exists before 4.3.0, so the client keeps it. Everything upstream added and this server does not yet cover is listed in #386.
...
- **`_actions` no longer suggests tools the server does not have** (#390). The next-step affordance was built from static lists, so a read-only server (`MCP_READONLY=true`) told clients to call `control` and `deployment` — tools it never registered. Suggestions are now filtered through the tools actually registered on that instance, in one place rather than at each call site. It matters more this release than last, because the new `instructions` field points the model at `_actions` explicitly.
